...
from common.types import LoginRequest, LoginResponse
...
from common.logging import get_logger  # ✅ 같은 로깅
from common.types import LogoutResponse  # ✅ 같은 타입
from common.errors import InvalidTokenError  # ✅ 같은 에러
...
# Task 1: Login
from common.logging import get_logger  # structlog
from common.types import LoginRequest   # Pydantic
logger = get_logger()

# Task 2: Logout (Task 1과 같은 패턴!)
from common.logging import get_logger  # ✅ 같은 structlog
...
Task 2: 개발자 B
- from common.logging import get_logger  # ✅ 같은 방식
...
|--------|----------|----------|----------|
| 타입/검증 | Pydantic v2 | 커스텀 검증 시스템 | common/types/ |
| 로깅 | structlog | print(), 커스텀 로거 | common/logging/ |
| 테스트 | pytest | unittest, 수동 mocking | tests/ |
| 설정 | pydantic-settings | 수동 환경변수 파싱 | common/config/ |
...
3단계: 표준 모듈 구축 (Common Modules)
  - common/logging/ 구현 (get_logger)
  - common/types/ 기본 모델
  - common/errors/ 예외 클래스
  - common/config/ 설정 관리
  - common/api/ 라우터 기본 구조
...
# Day 1 10:00-12:00: common/ 모듈 구현
...
**결과**: 이제 common/ 모듈 다 있음! → Blueprint 구체적으로 쓸 수 있음!
...
- **PROJECT_STANDARDS.md Line 56-71**: Pydantic 검증 패턴
  ...
  class TokenPayload(BaseModel):
      ...
      exp: datetime
  ```
- **ARCHITECTURE.md Line 23-45**: common/ 모듈 사용 규칙
...
from common.errors import InvalidUserError, TokenGenerationError
