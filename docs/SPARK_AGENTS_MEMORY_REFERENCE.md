...
## 🤖 전체 SPARK 에이전트 목록 (28개)

### 🎯 16개 기본 에이전트 

| 에이전트 | 역할 | 언제 사용 | 5-Phase 패턴 |
|---------|------|----------|-------------|
| **analyzer-spark** ⭐⭐⭐⭐⭐ | 다차원 시스템 분석 | 복잡한 분석, 문제 원인 파악 | Discovery → Evidence → Analysis → Testing → Synthesis |
| **designer-spark** ⭐⭐⭐⭐ | 포괄적 시스템 설계 | 아키텍처, API, UI 시스템 설계 | 요구분석 → 설계 → 검증 → 최적화 → 문서화 |
| **implementer-spark** ⭐⭐⭐⭐⭐ | 체계적 기능 구현 | 새 기능, API, 서비스 구현 | 계획 → 기반 → 구현 → 통합 → 검증 |
| **tester-spark** ⭐⭐⭐⭐ | 포괄적 테스트 우수성 | 테스트 작성, 품질 보장 | 계획 → 단위 → 통합 → E2E → 리포팅 |
| **documenter-spark** ⭐⭐⭐ | 전문적 문서 작성 | API 문서, 사용자 가이드 작성 | 분석 → 구조 → 작성 → 검토 → 배포 |
| **troubleshooter-spark** ⭐⭐⭐⭐ | 체계적 문제 해결 | 버그, 성능 문제, 장애 해결 | 증상분석 → 가설수립 → 증거수집 → 원인검증 → 솔루션설계 |
| **improver-spark** ⭐⭐⭐⭐⭐ | 체계적 코드 개선 | 기술부채, 리팩토링, 품질향상 | 분석 → 계획 → 구현 → 통합 → 검증 |
| **cleaner-spark** ⭐⭐⭐ | 기술부채 제거 | 죽은코드, 의존성, 레거시 정리 | 스캔 → 분류 → 정리 → 검증 → 최적화 |
| **builder-spark** ⭐⭐⭐ | 빌드 시스템 최적화 | CI/CD, 빌드 성능 개선 | 분석 → 설계 → 구현 → 테스트 → 배포 |
| **estimater-spark** ⭐⭐ | 증거기반 프로젝트 추정 | 개발시간, 리소스 추정 | 분석 → 분해 → 추정 → 검증 → 리포팅 |
| **loader-spark** ⭐⭐⭐ | 포괄적 프로젝트 온보딩 | 새 프로젝트 이해, 환경설정 | 탐색 → 분석 → 문서화 → 설정 → 검증 |
| **spawner-spark** ⭐⭐⭐ | 다중작업 조율 | 복잡한 멀티도메인 작업 | 분해 → 조율 → 실행 → 통합 → 검증 |
| **tasker-spark** ⭐⭐ | 엔터프라이즈 프로젝트 관리 | 장기 프로젝트, 팀 협업 관리 | 계획 → 분해 → 추적 → 조율 → 완료 |
| **gitter-spark** ⭐⭐ | Git 워크플로우 아키텍처 | 브랜치 전략, 커밋 규칙 설정 | 분석 → 설계 → 구현 → 테스트 → 배포 |
| **explainer-spark** ⭐⭐ | 교육 컨텐츠 생성 | 개념 설명, 학습 자료 작성 | 분석 → 구조화 → 작성 → 예시 → 검증 |
| **indexer-spark** ⭐ | 명령 카탈로그 탐색 | SuperClaude 명령 탐색, 추천 | 수집 → 분류 → 매칭 → 추천 → 가이드 |

### 👥 12개 팀 에이전트 (4개 팀 × 3개 역할)
...
**팀 에이전트 특징:**
- 🏃‍♂️ **병렬 실행**: 최대 4팀 동시 작업 (3.1배 속도 향상)
- 🔒 **FileLockManager**: 공유 파일 접근 자동 조율
- 📊 **독립 컨텍스트**: 각 팀별 `team{N}_task.json` 상태 관리
- ⚡ **확장성**: 독립적 작업 2-4개 최적

**팀 에이전트 사용법:**
```javascript
// ✅ 올바른 병렬 팀 사용 
Task("team1-implementer-spark", "Core Services 구현")
Task("team2-implementer-spark", "API Layer 구현") 
Task("team3-implementer-spark", "Database Layer 구현")
Task("team4-implementer-spark", "Frontend Components 구현")
```
