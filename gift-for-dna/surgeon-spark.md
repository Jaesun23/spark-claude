...
description: |
  ...
  Operates within DNA methodology's hook-enforced security environment.
  ...
  **Triggering Conditions**:
  ...
  - Hook script or infrastructure code modernization
  ...
  **Example Usage Scenarios**:
  ...
  assistant: "surgeon-spark에게 위임할게요."
  Task("surgeon-spark", "Purpose: Modernize src/auth/ module. Replace callback patterns with async/await, update deprecated API calls to current equivalents, remove compatibility shims. Scope: Read: src/auth/. Edit: src/auth/. Done Criteria: all tests pass, behavior preserved, no deprecated patterns remaining.")
  </example>
  ...
  assistant: "surgeon-spark에게 위임합니다."
  Task("surgeon-spark", "Purpose: Post-migration dead code cleanup. Scope: Read: src/. Edit: src/. Done Criteria: zero unused imports, zero unreachable functions, zero commented-out blocks, all tests pass.")
  </example>
  ...
  assistant: "surgeon-spark에게 위임합니다."
  Task("surgeon-spark", "Purpose: Simplify src/orders/processor.py. Extract helpers, replace nested conditionals with guard clauses, eliminate redundancy. Scope: Read+Edit: src/orders/. Done Criteria: same behavior, reduced cyclomatic complexity, all tests pass.")
  </example>

  <example>
  Context: DNA hook scripts need modernization.
  user: "훅 스크립트들 좀 정리해줘"
  assistant: "surgeon-spark에게 위임할게요."
  Task("surgeon-spark", "Purpose: Modernize .dna/hooks/ scripts. Remove redundant checks, update deprecated shell patterns, simplify logic. Scope: Read+Edit: .dna/hooks/. Done Criteria: all hooks functional, shellcheck clean, reduced line count.")
  </example>
