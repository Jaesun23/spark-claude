# 00. 핵심정의 가이드 (Core Definition Guide)
...
#### 1. **`01F-01_core_functions.md`** - 핵심 기능 정의
**내용**:
- 핵심 기능 1-3개 정의
- 각 기능의 실패 시 비즈니스 영향
- 구현 방식과 핵심 기능의 구분 명확히
...
## 핵심 기능 1: 거래
- **설명**: 주식 매수/매도 주문 실행
- **실패 시 영향**: 금전적 손실, 법적 책임
- **구현 방식**: 수동 거래, 자동 거래 (둘 다 동일한 핵심 기능)
```

#### 2. **`01C-01_family_classification.md`** - 패밀리 분류
**내용**:
- Layer 1 질문 결과 (L1-Q1, L1-Q2, L1-Q3)
- 패밀리 코드 (예: A-C-A)
- 패밀리 이름 (예: 실시간 트랜잭션)
- 각 질문의 근거와 선택 이유
...
## Layer 1 질문 결과
- L1-Q1 (실패 파급력): **A** (치명적) - 거래 실패 = 금전 손실
- L1-Q2 (정보 형태): **C** (숫자 데이터) - 가격, 수량, 시간
- L1-Q3 (응답 시점): **A** (밀리초) - 실시간 거래 필수

## 패밀리: A-C-A (실시간 트랜잭션)
```

#### 3. **`01C-02_nfr_profile.md`** - NFR 프로파일
**내용**:
- Layer 2 질문 결과 (L2-Q1~Q4)
- NFR 프로파일 (예: A-B-B-A)
- 각 질문의 근거
...
## Layer 2 질문 결과
- L2-Q1 (핵심 품질): **A** - 정확성 최우선
- L2-Q2 (규모): **B** - 중간 규모
- L2-Q3 (데이터 노출): **B** - 제한적 공개
- L2-Q4 (데이터 최신성): **A** - 실시간 필수

## NFR 프로파일: A-B-B-A
```

#### 4. **`01D-01_tech_candidates.md`** - 기술 후보군
**내용**:
- 패밀리가 강제하는 필수 기술 방향
- 기술 후보군 목록 (구체적 선택은 Stage 2에서)
...
- API 프레임워크: FastAPI, Node.js
- 캐시: Redis
- DB: PostgreSQL, TimescaleDB
- 프론트: React, Vue

**주의**: 구체적 선택은 Stage 2에서!
```
...
- `docs/DNA_Methodology_v4.0_Guide/family-tech-matrix/{패밀리}_tech_options.md` 링크
...
## Layer 1. DNA 방법론의 7가지 패밀리 유형

DNA Development Methodology v4.0은 모든 소프트웨어 시스템을 **3가지 핵심 차원**으로 분류합니다. 이론적으로는 2×3×3 = 18가지 조합이 가능하지만, 실증 연구를 통해 **실무적으로 의미 있고 검증된 7가지 패밀리**를 도출했습니다.

---

## 3가지 분류 차원 (3-Layer Decision Tree)

### 차원 1: 실패의 파급력 (Failure Impact)

**"시스템이 실패하면 어떤 일이 벌어지는가?"**
...
### 차원 2: 데이터의 형태 (Data Structure)

**"다루는 정보가 얼마나 정형화되어 있는가?"**
