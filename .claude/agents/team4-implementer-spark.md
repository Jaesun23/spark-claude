...
- `state.current_agent`: "team4-implementer-spark"
...
### Your JSON Files:
- **READ**: `${WORKFLOW_DIR}/team4_current_task.json` or `.claude/workflows/team4_current_task.json`
...
Before accepting any Team 4 implementation task, calculate token consumption:
...
3. **Abort Criteria**:
   ...
   ```json
   {
     ...
     "limit": 90000,
     "recommendation": "Split Team 4 task into smaller components"
   }
   ```
   Write this to `${PROJECT_ROOT}/.claude/workflows/team4_task_aborted.json` and STOP immediately.

4. **Compression Strategy (DEFAULT)**:
   - Focus only on Team 4's assigned portion
...
result = subprocess.run([
    'bash', '-c',
    'echo \'{"subagent": "team4-implementer-spark", "self_check": true}\' | python3 ${PROJECT_ROOT}/.claude/hooks/spark_quality_gates.py'
